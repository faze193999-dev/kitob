...
export const Categories: React.FC<CategoriesProps> = ({ books }) => {
  ...
  return (
    <div id="discover" className="py-16 border-t border-black/5">
              ...
              <motion.button
                ...
                className={`p-5 rounded-2xl border text-left cursor-pointer transition-all duration-300 relative overflow-hidden group ${
                  isSelected
                    ...
                    : "bg-black/5 border-transparent text-zinc-500 hover:text-zinc-950 hover:bg-black/8"
                }`}
                whileHover={{ y: -4 }}
                whileTap={{ scale: 0.98 }}
          ...
          <AnimatePresence mode="popLayout">
            {filteredBooks.map((book) => (
              <motion.div
                ...
                className="group relative glass-card rounded-3xl p-5 border border-black/5 flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="w-full aspect-[2/1] rounded-2xl overflow-hidden relative border border-black/5 mb-4 bg-zinc-100 flex items-center justify-center">
                    ...
                    <span className="absolute top-3 left-3 px-2 py-0.5 rounded bg-black/40 backdrop-blur-md text-[10px] text-zinc-200 font-medium border border-transparent uppercase tracking-wider">
                      {book.category}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-1 text-xs text-zinc-500 mb-1.5">
                    ...
                    <span className="flex items-center gap-0.5">
                      ...
                      <span className="text-zinc-650">{book.rating}</span>
                    </span>
                  </div>

                  <h3 className="text-lg font-semibold text-zinc-800 truncate group-hover:text-violet-700 transition-colors duration-200">
                    {book.title}
                  </h3>
